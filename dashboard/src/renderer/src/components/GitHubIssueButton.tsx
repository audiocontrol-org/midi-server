...
const GITHUB_REPO = 'audiocontrol-org/midi-server'

function formatTimestamp(timestamp: number): string {
  return new Date(timestamp).toISOString()
}

export function GitHubIssueButton({ buildInfo, logs }: GitHubIssueButtonProps): React.JSX.Element {
...
`
  ...
  return (
    <button
      onClick={handleClick}
      className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded text-gray-300 text-sm transition-colors cursor-pointer flex items-center gap-2"
    >
